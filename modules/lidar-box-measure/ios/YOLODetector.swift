...
/// YOLO11n ONNX inference wrapper (Boxer3D approach).
final class YOLODetector {
    ...
    /// Run YOLO detection on a CHW float32 array in [0,1], length = 3 * 640 * 640.
    func detect(
        image: [Float],
        imageWidth: Int = 640,
        imageHeight: Int = 640,
    ...
    private func nms(boxes: [YOLOBox], iouThreshold: Float) -> [YOLOBox] {
        ...
        var keep: [YOLOBox] = []
        for box in sorted {
            let overlaps = keep.contains { iou(box, $0) > iouThreshold && box.label == $0.label }
            if !overlaps { keep.append(box) }
        }
        return keep
    }

    private func iou(_ a: YOLOBox, _ b: YOLOBox) -> Float {
        let x1 = max(a.xmin, b.xmin), y1 = max(a.ymin, b.ymin)
        let x2 = min(a.xmax, b.xmax), y2 = min(a.ymax, b.ymax)
