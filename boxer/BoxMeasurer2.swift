...
///
/// Key fix vs 929917f:
///   dFront is now the MEDIAN of the centre third of the YOLO bbox
///   (not the 10th-percentile of the full bbox, which picked up floor edges).
///   dCut tightened from 1.00 m to 0.65 m to reduce wall/shelf contamination.
struct BoxMeasurer2 {
    ...
    static func measure(
        ...
        // 5. If box is viewed nearly face-on, fall back to background sampling for depth.
    ...
    private static func unproject(
        ...
        // Pass 1: dFront = MEDIAN of CENTRE THIRD of bbox (avoids floor at bottom edge).
        // Original 929917f used 10th-percentile of full bbox — picked up floor pixels at
        // the bottom of the bbox, placing the wireframe too close to the camera.
        let pyMid0 = py0 + (py1-py0)/3
        let pyMid1 = py0 + 2*(py1-py0)/3
        let pxMid0 = px0 + (px1-px0)/3
        let pxMid1 = px0 + 2*(px1-px0)/3
        var centreD: [Float] = []
        for py in pyMid0...pyMid1 {
            let row = base.advanced(by: py*rb).assumingMemoryBound(to: Float32.self)
            for px in pxMid0...pxMid1 {
                let d = row[px]; if d > 0.1 && d < 7 { centreD.append(d) }
            }
        }
